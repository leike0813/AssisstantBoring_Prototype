...
def calculate_grain_dist_parameters(cumulative_percentage, grain_size_group, ci_size_group):
    assert len(cumulative_percentage) == len(grain_size_group), 'The length of input cumulative percentage must be equal to the length of grain size group'
    _grain_size_group = [0, ]
    _cumulative_percentage = [0.0, ]
    for i in range(len(grain_size_group)):
        if cumulative_percentage[i] < (cumulative_percentage[i - 1] if i > 0 else 0.0):
            ...
            _grain_size_group.append(grain_size_group[i])
        else:
    ...
    with warnings.catch_warnings():
        ...
        cu = d60 / d10
        cc = d30 ** 2 / (d10 * d60)
        ci = sum([(1 - grain_dist_func(i)) * 100 for i in ci_size_group])

    return {'Cu': cu, 'Cc': cc, 'CI': ci, 'D10': d10, 'D30': d30, 'D50': d50, 'D60': d60}


def rearrange_statistics_dict(statistics_dict):
    def dummy(input): return input
    if isinstance(statistics_dict['dist_curve_area'].iloc[0], dict):
        eval_func = dummy
    elif isinstance(statistics_dict['dist_curve_area'].iloc[0], str):
        eval_func = literal_eval
    else:
        raise ValueError('Data type of distribution curve is invalid')

    all_cols = [
        'capture_time', 'slag_count', 'max_area', 'min_area', 'avg_area', 'total_area',
        'max_length', 'min_length', 'avg_length',
        'max_perimeter', 'min_perimeter', 'avg_perimeter',
        'max_sphericity', 'min_sphericity', 'avg_sphericity',
        'max_slenderness', 'min_slenderness', 'avg_slenderness',
        'max_volume', 'min_volume', 'avg_volume', 'total_volume',
        'Cu_area', 'Cc_area', 'CI_area', 'D10_area', 'D30_area', 'D50_area', 'D60_area', 'dist_curve_area',
        'Cu_volume', 'Cc_volume', 'CI_volume', 'D10_volume', 'D30_volume', 'D50_volume', 'D60_volume', 'dist_curve_area'
    ]
    max_cols = ['max_area', 'max_length', 'max_perimeter', 'max_sphericity', 'max_slenderness', 'max_volume']
    min_cols = ['min_area', 'min_length', 'min_perimeter', 'min_sphericity', 'min_slenderness', 'min_volume']
    avg_cols = ['avg_area', 'avg_length', 'avg_perimeter', 'avg_sphericity', 'avg_slenderness', 'avg_volume']
    sum_cols = ['slag_count', 'total_area', 'total_volume']
    drop_cols = [
        'image_name', 'image_width', 'image_height',
        'median_area', 'up_quartile_area', 'lo_quartile_area',
        'median_length', 'up_quartile_length', 'lo_quartile_length', 'total_length',
        'median_perimeter', 'up_quartile_perimeter', 'lo_quartile_perimeter', 'total_perimeter',
        'median_sphericity', 'up_quartile_sphericity', 'lo_quartile_sphericity', 'total_sphericity',
        'median_slenderness', 'up_quartile_slenderness', 'lo_quartile_slenderness', 'total_slenderness',
        'median_volume', 'up_quartile_volume', 'lo_quartile_volume',
    ]
    if 'id' in statistics_dict.columns:
        drop_cols.append('id')

    new_dict = pd.DataFrame(columns=statistics_dict.columns).astype(statistics_dict.dtypes)
    new_dict.drop(columns=drop_cols, inplace=True)
    new_idx = 0

    statistics_dict.sort_values(by='capture_time', inplace=True)
    statistics_dict = statistics_dict.loc[statistics_dict['slag_count'] > 0]


    _idx = 1
    sub_df_idx = [0]
    prev_time = statistics_dict['capture_time'].iloc[0].replace(microsecond=0)
    with warnings.catch_warnings():
        warnings.filterwarnings('ignore', category=pd.errors.SettingWithCopyWarning)
        warnings.filterwarnings('ignore', category=FutureWarning)

        while _idx <= len(statistics_dict):
            if _idx == len(statistics_dict):
                new_time = datetime.datetime.fromordinal(1)
            else:
                new_time = statistics_dict['capture_time'].iloc[_idx].replace(microsecond=0)
            if new_time == prev_time:
                sub_df_idx.append(_idx)
            else:
                if len(sub_df_idx) == 1:
                    new_dict.loc[new_idx] = statistics_dict.iloc[sub_df_idx[0]]
                else:
                    sub_df = statistics_dict.iloc[sub_df_idx]
                    new_dict.loc[new_idx] = np.nan
                    for _col in sum_cols:
                        new_dict[_col].loc[new_idx] = sub_df[_col].sum()
                    for _col in max_cols:
                        new_dict[_col].loc[new_idx] = sub_df[_col].max()
                    for _col in min_cols:
                        new_dict[_col].loc[new_idx] = sub_df[_col].min()
                    for _col in avg_cols:
                        new_dict[_col].loc[new_idx] = sub_df.apply(lambda row: row[_col] * row['slag_count'], axis=1).sum() / new_dict['slag_count'].loc[new_idx]
                    total_dist_curve_area = {k: 0.0 for k in eval_func(sub_df['dist_curve_area'].iloc[0]).keys()}
                    total_dist_curve_volume = {k: 0.0 for k in eval_func(sub_df['dist_curve_volume'].iloc[0]).keys()}
                    for sidx in range(len(sub_df_idx)):
                        for k in total_dist_curve_area.keys():
                            total_dist_curve_area[k] += eval_func(sub_df['dist_curve_area'].iloc[sidx])[k] * sub_df['total_area'].iloc[sidx]
                        for k in total_dist_curve_volume.keys():
                            total_dist_curve_volume[k] += eval_func(sub_df['dist_curve_volume'].iloc[sidx])[k] * sub_df['total_volume'].iloc[sidx]
                    for k in total_dist_curve_area.keys():
                        total_dist_curve_area[k] /= new_dict['total_area'].loc[new_idx]
                    for k in total_dist_curve_volume.keys():
                        total_dist_curve_volume[k] /= new_dict['total_volume'].loc[new_idx]
                    new_dict['dist_curve_area'].loc[new_idx] = str(total_dist_curve_area)
                    new_dict['dist_curve_volume'].loc[new_idx] = str(total_dist_curve_volume)
                    dist_parameters_area = calculate_grain_dist_parameters(list(total_dist_curve_area.values())[:-1],
                                                                           list(total_dist_curve_area.keys())[:-1])
                    dist_parameters_volume = calculate_grain_dist_parameters(list(total_dist_curve_volume.values())[:-1],
                                                                             list(total_dist_curve_volume.keys())[:-1])
                    for k in dist_parameters_area.keys():
                        new_dict['{}_area'.format(k)].loc[new_idx] = dist_parameters_area[k]
                    for k in dist_parameters_volume.keys():
                        new_dict['{}_volume'.format(k)].loc[new_idx] = dist_parameters_volume[k]

                new_dict['capture_time'].loc[new_idx] = prev_time
                new_idx += 1
                sub_df_idx = [_idx]
                prev_time = new_time
            _idx += 1
