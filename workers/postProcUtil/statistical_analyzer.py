...
from .pproc_functions import (
    ...
    calculate_grain_dist_parameters,
)
...
class StatisticalAnalyzer:
    def __init__(self, pixel_ratio=0.44, grain_size_group=[5, 10, 20, 30, 40, 50, 60, 80, 100, 120, 150, 200, 250, 300],
                 redilate_iter=2, kernel_shape='ellipse', kernel_size=3, ci_size_group=[25, 50, 75, 100, 125, 150, 175, 200]):
        self.pixel_ratio = pixel_ratio
        self.grain_size_group = grain_size_group
        self.ci_size_group = ci_size_group
    ...
    def calculate_statistics(self, contour_dict, image_size):
        ...
        statistic_dict['slag_count'] = len(contour_dict)
    ...
    def calculate_grain_distribution(self, length_series, area_series, volume_series):
        _merged_grain_size_group = sorted(list(set(copy(self.grain_size_group)).union(set(copy(self.ci_size_group)))))
        index = copy(_merged_grain_size_group)
        index.append(10000)
        grain_dist_dict = pd.DataFrame(columns=['area_sum', 'volume_sum'], index=index)
        ...
        grain_dist_parameters = {
            'area': calculate_grain_dist_parameters(list(grain_dist_dict['cum_area_percentage'].values)[:-1],
                                                    _merged_grain_size_group, self.ci_size_group),
