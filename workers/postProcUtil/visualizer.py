...
class Visualizer:
    ...
    def __init__(self, visualization_mode='log_area', visualization_alpha=0.5, colormap='turbo',
                 draw_contours=False, draw_legend=True, draw_graindist=True,
                 draw_statistics=True, legend_and_graphs=None, result_format='png'):
    ...
    def __call__(self, orig_image, contour_dict, statistic_dict, grain_dist_dict, grain_dist_parameters):
        ...
        result_image = self.draw_result(orig_image, contour_dict)
        result_image = self.draw_legend_and_graphs(result_image, contour_dict, statistic_dict, grain_dist_dict, grain_dist_parameters)

        return result_image, orig_image
    ...
    def draw_legend_and_graphs(self, image, contour_dict, statistic_dict, grain_dist_dict, grain_dist_parameters):
        if self.visualization_mode == VisualizationMode.ID:
            return {'result.{}'.format(self.result_format): image}
        # -----------parameters------------------
        ...
        graph_anchor_left = (legend_anchor[0] + legend_size[0] + graph_spacing, legend_anchor[1])
        graph_anchor_right = (
            legend_anchor[0] + legend_size[0] + graph_spacing,
